...
TAOBAO_CHANNEL_KEY = [u'ttc_ws_waigou',
                      u'ttc_ws_ziyou',
                      u'ttc_ps_waigou',
                      u'ttc_ps_ziyou']

TAOBAO_CHANNEL = {TAOBAO_CHANNEL_KEY[0]: u'ws_waigou',
                  TAOBAO_CHANNEL_KEY[1]: u'ws_ziyou',
                  TAOBAO_CHANNEL_KEY[2]: u'ps_waigou',
                  TAOBAO_CHANNEL_KEY[3]: u'ps_ziyou'}
    ...
    def process(self, tup):
        ...
        if self.loglist: 
            ...
            if self.redis.set(self.rkey, 1, ex=600, nx=True):
                ...
                self.class_level1 = u'UNKNOW' if self.class_info[0] is None else self.class_info[0]
                self.class_level2 = u'UNKNOW' if self.class_info[0] is None else self.class_info[1]
                self.class_level3 = u'UNKNOW' if self.class_info[0] is None else self.class_info[2]
                ...
                self.taobao_channel = u'UNKNOW' if self.tmp is None else self.tmp

                if self.more_values_count >= self.write_msg_to_db_line:

                    #self.more_values_count = 0
                    #self.more_values = []
                    a = "\n".join(self.more_values)
                    #self.logger.info(a)
                    #self.influxdb.write_points(a, protocol='line')
                    self.conndb.write_influxdb(a)
                    self.more_values_count = 0
                    self.more_values = []
                    #self.logger.info('====='*50)
                    ...
                    data = u"wsielog,tabao_channel=%s,businesslevel1=%s,businesslevel2=%s,businesslevel3=%s,serverip=%s,sourceip=%s,pid=%s,optime=%d,logtime=%s,cost=%s,taobao_accountid_count=%s,business=%s adid_count=%s"%(\
                                    self.taobao_channel,\
                                    self.class_level1,\
                                    self.class_level2,\
                                    self.class_level3,\
