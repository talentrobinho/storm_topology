...
class FilterLogBigCustomerBolt(Bolt):
    '''
    ...
    outputs = [Stream(fields=['timestamp', 'pid', 'accountid', 'effclick', 'consume', 'clickid', 'srcip', 'bussines', 'queryflag', 'server_ip'], name='filter_big_customer_log_stream')]
                ...
                if self.loglist[6] in self.account_result_set:
                    ...
                    if int(self.loglist[12]) == 0:
                        ...
                        self.msg = [self.loglist[0], 
                                    ...
                                    self.loglist[34], 
                                    self.server_ip]
                        #self.redis.publish("watch_log", "{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}".format(time.time(),self.loglist[0], self.loglist[2], self.loglist[6], self.loglist[12], self.loglist[14], self.loglist[22], self.loglist[34]))
                        #self.emit(["{}\t{}\t{}\t{}\t{}\t{}\t{}".format(loglist[0], loglist[2], loglist[6], loglist[12], loglist[14], loglist[22], loglist[34])], stream='filter_log_stream')
                        #self.emit(self.msg, stream='xxtaobao_filter_log_stream')
                        self.emit(self.msg, stream='filter_big_customer_log_stream')
                        #self.logger.info("---{0}---".format(','.join(self.msg)))
                        #self.emit(tup.values[0], tup_id=tup.id, stream='filter_log_stream')
                #else:
                #    self.redis.publish("watch_log", "{}".format(self.loglist[6]))
                    #self.logger.error("------------ accountid {0} is not in account result.------------".format(self.loglist[6]))
