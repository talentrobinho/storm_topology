...
class FilterLogInputGuangBolt(Bolt):
    '''
    ...
    outputs = [Stream(fields=['timestamp', 'pid', 'accountid', 'effclick', 'consume', 'clickid', 'srcip', 'bussines', 'queryflag', 'server_ip'], name='filter_input_guang_log_stream')]
            ...
            if len(self.loglist) < self.log_length:
                ...
                if self.loglist[6] in self.account_result_set:
                    ...
                    if int(self.loglist[12]) == 0:
                        ...
                        self.emit(self.msg, stream='filter_input_guang_log_stream')
