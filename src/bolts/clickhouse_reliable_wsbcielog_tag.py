...
class FilterLogClickHouseBolt(Bolt):
    ...
    outputs = [Stream(fields=["time",
                              ...
                              "sogouhao_searchkey",
                              "sgh_tag",
                              "gen_log_ip",], name='clickhouse_tag_wsbcielog_stream')]
    ...
    def process(self, tup):
            ...
            if info_len < self.log_length:
                ...
                self.emit(new_loglist, stream='clickhouse_tag_wsbcielog_stream')
            else:
                self.loglist.append(self.server_ip) 
                #self.logger.info(self.loglist) 
                self.emit(self.loglist, stream='clickhouse_tag_wsbcielog_stream')
