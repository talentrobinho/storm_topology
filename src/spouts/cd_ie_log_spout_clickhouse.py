...
class CdIeLogClickHouseSpout(Spout):
    #outputs = ['cd_ie_log', 'server_ip']
    outputs = [Stream(fields=['cd_ie_log', 'server_ip'], name='search_clickhouse_spout')]
    ...
    def next_tuple(self):
        ...
        if not self.line_value:
            ...
            self.emit([self.message, self.server_ip], stream='search_clickhouse_spout')
        else:   
            self.server_ip = self.line.key().split('_')[0]
            for line in self.line_value.split('\n'):
                self.emit([line, self.server_ip], stream='search_clickhouse_spout')
