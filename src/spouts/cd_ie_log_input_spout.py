...
class CdIeLogInputSpout(Spout):
    #outputs = ['cd_ie_log', 'server_ip']
    outputs = [Stream(fields=['cd_ie_log', 'server_ip'], name='input_spout')]
    ...
    def next_tuple(self):
        ...
        if not self.line_value:
            ...
            self.emit([self.message, self.server_ip], stream='input_spout')
            ...
            for line in self.line_value.split('\n'):
                self.emit([line, self.server_ip], stream='input_spout')
