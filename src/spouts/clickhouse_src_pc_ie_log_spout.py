...
class PCIeLogSpout(Spout):
    #outputs = ['cd_ie_log', 'server_ip']
    outputs = [Stream(fields=['pc_ie_log', 'server_ip'], name='pcielog_spout')]
    ...
    def initialize(self, stormconf, context):
        ...
        self.kafka_group = 'pct1'
        self.kafka_subscribe = 'pcielog'

        self.conndb = ConnDB()
        self.kafka = self.conndb.conn_js_kafka(self.kafka_group)
    ...
    def next_tuple(self):
        ...
        if self.line.error():
            self.logger.error("PcIeLogSpout error: {}".format(self.line.error()))
            ...
            for line in self.line_value.split('\n'):
                self.emit([line, self.server_ip], stream='pcielog_spout')
