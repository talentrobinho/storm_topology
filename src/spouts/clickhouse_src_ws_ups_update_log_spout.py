...
class WsUpdateUpdateLogSpout(Spout):
    #outputs = ['cd_ie_log', 'server_ip']
    outputs = [Stream(fields=['wsups_update_log', 'server_ip'], name='ups_updatelog_spout')]
    ...
    def initialize(self, stormconf, context):
        ...
        self.kafka_group = 'wsups'
        self.kafka_subscribe = 'wsup_updatelog'

        self.conndb = ConnDB()
        self.kafka = self.conndb.conn_new_kafka(self.kafka_group)
    ...
    def next_tuple(self):
        ...
        if self.line.error():
            self.logger.error("WsUpdateUpdateLogSpout error: {}".format(self.line.error()))
            ...
            for line in self.line_value.split('\n'):
                self.emit([line, self.server_ip], stream='ups_updatelog_spout')
