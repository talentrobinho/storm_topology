...
class WsLKIeLogSpout(Spout):
    #outputs = ['cd_ie_log', 'server_ip']
    outputs = [Stream(fields=['wslk_ie_log', 'server_ip'], name='wslkielog_spout')]
    ...
    def initialize(self, stormconf, context):
        ...
        self.kafka_group = 'wslk'
        self.kafka_subscribe = 'wslk_ielog'
    ...
    def next_tuple(self):
        ...
        if self.line.error():
            self.logger.error("WsLKIeLogSpout error: {}".format(self.line.error()))
            ...
            for line in self.line_value.split('\n'):
                self.emit([line, self.server_ip], stream='wslkielog_spout')
