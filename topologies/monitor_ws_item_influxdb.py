...
from bolts.analysis_ws_log_influxdb import *


class AnalysisWSLog(Topology):
    ws_log_spout = WSIeLogSpout.spec()
    ws_log_stream_bolt = FilterWSLogBolt.spec(inputs=[ws_log_spout],par=6)
    '''taobao calculate consume'''
    tag_ws_log_bolt = TagWSLogBolt.spec(inputs=[ws_log_stream_bolt['filter_ws_log_stream_influxdb']], par=18)
